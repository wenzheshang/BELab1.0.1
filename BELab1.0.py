...
5/Mar/2023
UDF & CAS support by Wenzhe Shang @TJU
29/NOV/2023
Real Model embeded by Wenzhe Shang @TJU
1/Dec/2023
===============================
...
JModelica or OpenModelica could be used to produce FMU file for our couple simulation;
"""
...
class Win(QWidget,Ui_Form):
    def __init__(self, parent = None):
        ...
        #预加载cas和dat文件，这部分主要是可以不需要加入边界条件也可以直接对该问题进行模拟
        def load_cas_dat():
            try:
                root = tk.Tk()
                root.withdraw()
            ...
            try:
                ...
                if self.NegitiveP.isChecked() == True:
                    if BoundaryName.find('inlet') != -1:
                        ...
                        scheme.doMenuCommand("/define/boundary/zone-type "+BoundaryName+' mass-flow-outlet')
        ...
        def startSimulation_click():
            try:
                ...
                for exd in range(len(EXD['FM'])):
                    ...
                    if FluentName.find('inlet_massflow') != -1:
                        scheme.doMenuCommandToString('/report/fluxes/mass-flow no inlet* () yes simu_'+run_id+'.txt')#保存到文件里
                        f = open(os.path.join(workPath,'simu_'+run_id+'.txt'), 'r', encoding='utf-8')
                        line = f.read()
                        item = line.split()
        ...
        def FMU_simulate(fmu_file, end_time, communicate_time, set_variable, set_value):
            ...
            for i in range(len(result)):
                allName += [model.get_variable_valueref(result[i])]
            ...
            allvariable = model.get_model_variables()
            ...
            while time < Tend and not model.get_event_info().terminateSimulation:
                ...
                if step_event or time_event or state_event:
                    ...
                    if number >= communicate_time:
                        ...
                        number = 0
                    number = number + dt #0.001 #不同模型需要的时间步长不同
                ...
                if cycleTime%100 == 0:
                    for i in range(len(result)):
                        save_result[result[i]].append(model.get_real([allName[i]])[0])
                        
            # model.get_model_variables()
            # res = model.simulate(final_time=720)
            # t = res['time']
            # x1 = res['SupplyAir.T_in']
            mydataframe_fmuResult = pd.DataFrame({'VarName':save_result.keys(),'Value':save_result.values()})
            mydataframe_fmuResult.to_csv(os.path.join(dir_result, 'fmuResult.csv'))
            ...
            # for i in range(len(result)):
            #     save_result[result[i]] = model.get_real([model.get_variable_valueref(result[i])])[0]

            # mydataframe = pd.DataFrame({'VarName':save_result.keys(),'Value':save_result.values()})
            # mydataframe.to_csv(os.path.join('a.csv'))
        ...
        self.casButton.pressed.connect(load_cas_dat)
        self.udfButton.pressed.connect(load_udf)
